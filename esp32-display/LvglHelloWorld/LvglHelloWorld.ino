...
void setup()
{
  ...
  else
  {
    ...
    lv_style_init(&st);
    lv_style_set_text_font(&st, &arial_40);//LV_FONT_MONTSERRAT_48);
    lv_style_set_text_color(&st, lv_palette_main(LV_PALETTE_BLUE));

    /* Create simple label */
    lv_obj_t *label = lv_label_create(lv_scr_act());
    lv_label_set_text(label, "Hello Arduino! Здрасти (V" GFX_STR(LVGL_VERSION_MAJOR) "." GFX_STR(LVGL_VERSION_MINOR) "." GFX_STR(LVGL_VERSION_PATCH) ")");
